...
function handleOrientation(event) {
  ...
  controls.x = THREE.MathUtils.lerp(
    controls.x,
    Math.max(-1, Math.min(1, tiltX)),
    0.1
  );
  controls.y = THREE.MathUtils.lerp(
    controls.y,
    Math.max(-1, Math.min(1, tiltY)),
    0.1
  );
}

if (/Mobi|Android|iPhone|iPad|iPod/i.test(navigator.userAgent)) {
  canvasContainer.addEventListener("click", requestDeviceOrientation, {
    once: true,
  });
}
...
function animate() {
  ...
  const positions = [];
  const colors = [];
  const particleColorHex = getComputedStyle(document.documentElement)
    .getPropertyValue("--stars-color")
    .trim();
  ...
  particles.children.forEach((particle, i) => {
    ...
    const twinkle =
      ...
      0.5 *
        Math.sin(
          time * particle.userData.twinkleSpeed + particle.userData.twinklePhase
        );
...
nameElement.innerHTML = `${firstName
  .split("")
  .map(
    (char, index) =>
      `<span class="char-wrapper" style="--char-index: ${index}">${char}</span>`
  )
  .join("")}<span class="char-wrapper" style="--char-index: ${
  firstName.length
}"> </span>${lastName
  .split("")
  .map(
    (char, index) =>
      `<span class="char-wrapper" style="--char-index: ${
        firstName.length + 1 + index
      }">${char}</span>`
  )
  .join("")}`;
...
gsap.utils.toArray(".education-card").forEach((card) => {
  gsap.from(card, {
    ...
    y: 100,
    duration: 1,
    scrollTrigger: { trigger: card, start: "top 80%", ease: "power2.out" },
  });
});
gsap.utils.toArray(".skill-item").forEach((item) => {
  gsap.from(item, {
    ...
    y: 50,
    duration: 0.8,
    scrollTrigger: { trigger: item, start: "top 80%", ease: "power2.out" },
  });
});
...
gsap.from(".social-icons a", {
  ...
  stagger: 0.2,
  duration: 0.5,
  scrollTrigger: { trigger: ".connect", start: "top 80%", ease: "power2.out" },
});

// Add this after your existing gallery modal code
document.querySelectorAll(".project-image").forEach((image) => {
  image.addEventListener("click", () => {
    ...
    modalCaption.textContent = image.alt; // Use alt text as caption
...
document.addEventListener("DOMContentLoaded", () => {
  if (typeof emailjs === "undefined") {
    console.error(
      "EmailJS library not loaded. Check the script source in HTML."
    );
